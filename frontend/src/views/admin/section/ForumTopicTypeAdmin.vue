...
<template>
  <div>
    <div class="topic-type-header">
      <div>
        <div class="title">论坛帖子类型管理</div>
        <div class="desc">在这里管理论坛所有的帖子类型</div>
      </div>
      <el-button type="primary" :icon="Plus" @click="createTopic"
        >新建</el-button
      >
    </div>
    <el-table :data="types" height="400">
      ...
      <el-table-column prop="name" label="类型名称" width="200" align="center">
        <template #default="{ row }">
          <div class="topic-type">
            <div
              class="type-dot"
              :style="{ backgroundColor: findType(row.id)?.color ?? '#bababa' }"
            ></div>
      ...
      <el-table-column label="操作" width="150" fixed="right" align="center">
        <template #default="{ row }">
          <el-button size="small" type="info" @click="startEdit(row)" plain
            >编辑</el-button
          >
          <el-button size="small" type="danger" @click="deleteType(row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="类型名称">
          <div style="display: flex; gap: 10px; align-items: center">
            <el-input v-model="editor.target.name" style="flex: 1" />
            <div style="display: flex; align-items: center; gap: 5px">
              <span style="font-size: 13px; color: #606266">颜色：</span>
              <el-color-picker
                v-model="editor.target.color"
                size="default"
                show-alpha="false"
                format="hex"
              />
