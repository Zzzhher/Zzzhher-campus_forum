...
public class FlowUtils {
    ...
    /**
     * 检查每日计数限制（当天发帖+评论总数限制）
     * 只检查不增加计数
     * 
     * @param counterKey 计数键
     * @param limit      每日限制数量
     * @return 是否通过限流检查（true表示未超过限制，false表示已超过限制）
     */
    public boolean checkDailyLimit(String counterKey, int limit) {
     ...
     * @param counterKey 计数键
     */
    public void incrementDailyCounter(String counterKey) {
        String countStr = template.opsForValue().get(counterKey);
        if (countStr != null) {
            template.opsForValue().increment(counterKey);
        } else {
            template.opsForValue().set(counterKey, "1");
            long secondsUntilMidnight = getSecondsUntilMidnight();
            template.expire(counterKey, secondsUntilMidnight, TimeUnit.SECONDS);
        }
    }
